...
def _get_windspeed_core(
    ...
    params = {"lat": lat, "lng": lng, "height": height, "avg_type": avg_type}
...
def get_windspeed(
    ...
    try:
        if ensemble:
            return _get_windspeed_core(
                lat, lng, height, "global", source="athena_ensemble"
            )
        else:
...
def _get_energy_production_core(
    ...
    params = {"lat": lat, "lng": lng, "height": height, "avg_type": "none"}
    ...
    if time_period == "global":
        summary_avg_energy_production = (
            power_curve_manager.fetch_avg_energy_production_summary(
        ...
        yearly_avg_energy_production = (
            power_curve_manager.fetch_yearly_avg_energy_production(
                df, height, selected_powercurve
            )
        )
