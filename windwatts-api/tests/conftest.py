...
def pytest_configure(config):
    ...
    def create_mock_windwatts_client():
        ...
        mock_client.fetch_global_avg_at_height = MagicMock(return_value={
            'global_avg': 8.60
        })
        
        mock_client.fetch_yearly_avg_at_height = MagicMock(return_value={
            'yearly_avg': [
        ...
        mock_client.fetch_monthly_avg_at_height = MagicMock(return_value={
            'monthly_avg': [
        ...
        mock_client.fetch_hourly_avg_at_height = MagicMock(return_value={
            'hourly_avg': [
        ...
        mock_client.calculate_summary_energy = MagicMock(return_value={
            'Lowest year': {'year': 2023.0, 'Average wind speed (m/s)': '8.12', 'kWh produced': 504212},
            'Average year': {'year': None, 'Average wind speed (m/s)': '8.60', 'kWh produced': 539072},
            'Highest year': {'year': 2013.0, 'Average wind speed (m/s)': '8.93', 'kWh produced': 579146}
        })
        
        mock_client.calculate_yearly_energy = MagicMock(return_value={
        ...
        mock_client.calculate_monthly_energy = MagicMock(return_value={
        ...
        def mock_query_athena(query, convert_to_dataframe=True, *args, **kwargs):
            if convert_to_dataframe:
                ...
                return [['wind_speed_30m', 'wind_speed_40m', 'wind_speed_50m', 
                        'wind_speed_60m', 'wind_speed_80m', 'wind_speed_100m']]
        ...
        def mock_fetch_df(lat, long, height, *args, **kwargs):
            ...
            return pd.DataFrame({
                ...
                'mohr': timestamps.month * 100 + timestamps.hour  # month-hour combined
            })
        ...
        mock_client.find_n_nearest_locations = MagicMock(side_effect=mock_find_n_nearest)
        
        # Set available heights (including 10m for legacy WTK endpoints)
        mock_client.available_heights = [10, 30, 40, 50, 60, 80, 100, 120, 140, 160, 200]
        mock_client.column_names = ['wind_speed_10m', 'wind_speed_30m', 'wind_speed_40m', 
                                    'wind_speed_50m', 'wind_speed_60m', 'wind_speed_80m', 
                                    'wind_speed_100m', 'wind_speed_120m', 'wind_speed_140m',
                                    'wind_speed_160m', 'wind_speed_200m']
        
        return mock_client
    
    # Patch the windwatts_data client classes
    wtk_patch = patch('windwatts_data.WindwattsWTKClient', return_value=create_mock_windwatts_client())
    era5_patch = patch('windwatts_data.WindwattsERA5Client', return_value=create_mock_windwatts_client())
    ensemble_patch = patch('windwatts_data.WindwattsEnsembleClient', return_value=create_mock_windwatts_client())
